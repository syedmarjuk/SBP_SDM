# =============================================================================
# 06_evaluation.R (VERSION 8b - THE 5KM EXCLUSION BUFFER)
# -----------------------------------------------------------------------------
# PURPOSE:
#   Evaluate the elite v8 model (8 variables, RM 3.0) using a modified
#   background strategy. A 5km spatial buffer is drawn around all known
#   occurrences, and the KDE bias surface is masked to FORBID background
#   points from landing inside these pelican hotspots.
#
# OUTPUTS (all saved to data/v8b/evaluation/):
...
test_occ_file     <- here("data", "processed", "test_thinned_with_year.csv")
...
out_dir <- here("data", "v8b", "evaluation")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# ---------------------------------------------------------------------------
# 2. FIT V8 MODEL (IN-MEMORY)
# ---------------------------------------------------------------------------
message("Fitting RM 3.0 maxnet model from v8 SWD CSVs...")
...
# ---------------------------------------------------------------------------
# 3. LOAD RASTERS & EXTRACT TEST DATA (FULL AOI)
# ---------------------------------------------------------------------------
message("Loading rasters and extracting test data...")
...
pts_test_v <- terra::vect(test_occ, geom = c("decimalLongitude", "decimalLatitude"), crs = "EPSG:4326")

test_env <- data.frame(terra::extract(bio_test[[bio_vars]], pts_test_v, ID = FALSE))
test_env$DistCoastalWet <- terra::extract(snap_dcw, pts_test_v, ID = FALSE)[, 1]
test_env$JRC            <- terra::extract(snap_jrc, pts_test_v, ID = FALSE)[, 1]
test_env$GLC            <- as.integer(terra::extract(snap_glc, pts_test_v, ID = FALSE)[, 1])
test_env$GWL            <- as.integer(terra::extract(snap_gwl, pts_test_v, ID = FALSE)[, 1])
test_swd <- test_env[complete.cases(test_env[, all_vars]), ]

# ---------------------------------------------------------------------------
# 4. THE EXCLUSION BUFFER & BACKGROUND SAMPLING
# ---------------------------------------------------------------------------
message("\n--- EXCLUSION BUFFER PROTOCOL ---")
...
occ_buffer       <- terra::buffer(pts_all_v, width = 5000)
bias_test_masked <- terra::mask(bias_test_r, occ_buffer, inverse = TRUE)

bg_pts   <- terra::spatSample(bias_test_masked, size = 10000,
...
message("\n=============================================================")
message("  v8b (5km EXCLUSION BUFFER) — EVALUATION RESULTS")
message("=============================================================")
...
p_roc <- ggplot(roc_df, aes(x = FPR, y = TPR)) +
  ...
  labs(title    = "ROC Curve — v8b (5km Exclusion Buffer)",
       subtitle = "Independent test data 2011-2021 | Full AOI",
...
fit_and_eval <- function(vars_subset, fc = "lq", rm = 3.0) {
  # Subset training, test, bg to requested variables
...
# Add full-model row (labelled " " to sit at bottom of Y axis like MaxEnt)
...
# --- PLOT ---
# Y axis: variables alphabetical top-to-bottom, " " (full model) at the very bottom
...
p_jack <- ggplot() +
  ...
  labs(
    title    = "Jackknife of Test AUC for Pelecanus_philippensis (v8b)",
    x        = "Test AUC (2011-2021)",
    y        = "Environmental Variable"
  ) +
  # Legend built with annotate to avoid any interaction with fill aesthetics
  annotate("rect",
...
# ---------------------------------------------------------------------------
report <- c(
  "MODEL EVALUATION REPORT (v8b — 5km Exclusion Buffer)",
  paste0("Date        : ", Sys.time()),
  paste0("Script      : v8b_07_evaluation.R"),
  strrep("=", 55),
  "",
  ...
  "",
  "BACKGROUND STRATEGY:",
  "  10,000 pts from KDE bias surface (2011-2021)",
...
message("\n  v8b COMPLETE — all outputs in data/v8b/evaluation/")
